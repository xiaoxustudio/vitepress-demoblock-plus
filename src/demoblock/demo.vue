<template>
  <div :class="['demo', hover && 'demo-hover']" @mouseenter="hover = true" @mouseleave="hover = false">
    ...
    <div class="demo-show" v-show="isExpanded">
      ...
      <!-- 代码 -->
      <div :class="['demo-show-code', 'language-' + language]" @mouseenter="codehover = true"
        @mouseleave="codehover = false">
        <!-- 复制 -->
        <transition name="fade">
          <div v-show="codehover" class="demo-show-switch" @click.stop="onClickControl">收</div>
        </transition>
        <transition name="fade">
          <div v-show="codehover" class="demo-show-copy" @click.stop="onCopy"></div>
        </transition>
        <span style="width: inherit;height: inherit;" v-html="decodedHtmlStr"></span>
      </div>
    </div>
    <!-- 按钮控制 -->
...
<script setup lang="ts">
  ...
  const { codePath, codeStr, htmlStr, description, language } = withDefaults(defineProps<Prop>(), {
    language: () => 'vue'
  })
  const slots = useSlots();
  const hover = ref(false); //鼠标是否悬浮之上
  const codehover = ref(false); //鼠标是否code悬浮之上
  const isExpanded = ref(false); // 展开or收起代码
  ...
  // 注册演示组件
  const demoSlot =
    codePath ? defineAsyncComponent(() => import(codePath)) : null;
  ...
  // 复制代码
  const onCopy = async (e: { target: any; }) => {
    try {
